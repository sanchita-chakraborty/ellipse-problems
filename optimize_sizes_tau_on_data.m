function [a_best, b_best, phi_best, tau_best, hist] = optimize_sizes_tau_on_data(CH, C_best, varargin)
...
p.addParameter('SepMargin', 4.0);   % minimum extra gap between ellipses (USED)
...
p.addParameter('WSep', 1e3);   % weight for overlap/separation penalty
...
% ---- fixed centers from data (nuclear positions) ----
C0   = [E.cx(1:N).', E.cy(1:N).'];   % N×2
...
function f = wrapper(psx)
    ...
    % (c) explicit pairwise overlap/separation
    penSep = overlap_penalty(C0, aa, bb, opt.SepMargin);
    
    % combine penalties
    pen = opt.WNNProp*penNN + opt.WSizeProp*penBnd + opt.WSep*penSep;
